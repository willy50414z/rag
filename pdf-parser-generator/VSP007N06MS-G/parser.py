...
def _norm_symbol(raw: str) -> str:
    """Flatten subscript newlines and PUA, return clean symbol string."""
    s = _apply_pua(raw)
    s = s.replace("\n", "")
    return s.strip()
...
def find_table_by_header(pdf, expected_headers, search_pages=None, col_count=None):
    ...
    for page in pages:
        for ti, tbl_obj in enumerate(page.find_tables() or []):
            ...
            if all(h.lower() in header_joined.lower() for h in expected_headers):
...
def _extract_part_id(pdf) -> str | None:
    ...
    m = re.search(r"\b(VSP\w+(?:-G)?)\b", text)
...
def parse_parts(pdf, part_id: str) -> list[dict]:
    ...
    if table is None:
        ...
        text = page.extract_text() or ""
        m = re.search(
            r"(VSP\S+)\s+(PDFN\S+)\s+(\S+)\s+(\d+\S+/Reel)",
            text.replace("\n", " ")
        )
        if m:
            records.append({
...
def parse_max_ratings(pdf, part_id: str) -> list[dict]:
    ...
    if table is None:
        warnings.warn("P1-T2 (max_ratings) anchor not found; returning empty")
        return []
    ...
    for row in table[1:]:
        ...
        # Merge @VGS condition from parameter into condition string
        if param_vgs_cond:
...
def parse_thermal(pdf, part_id: str) -> list[dict]:
    ...
    if table is None:
        warnings.warn("P1-T3 (thermal_characteristics) anchor not found; returning empty")
        return []
...
def parse_electrical(pdf, part_id: str) -> list[dict]:
    ...
    if table is None:
        warnings.warn("P2-T0 (electrical_characteristics) anchor not found; returning empty")
        return []
    ...
    for row in table[1:]:
        ...
        # col layout: [Symbol, Parameter, Condition_primary, Condition_secondary, Min, Typ, Max, Unit]
        sym_raw    = cells[0] if len(cells) > 0 else ""
        param_raw  = cells[1] if len(cells) > 1 else ""
        ...
        if sym_raw:
            ...
            # Qg(xV) special case
            qg_match = re.match(r"Qg?\s*\((\d+\.?\d*V)\)", sym_norm_raw)
            if qg_match:
                extracted_vgs = qg_match.group(1)
                sym_norm = "Qg"
            else:
                extracted_vgs = None
